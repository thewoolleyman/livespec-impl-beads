# Four rehearsal receipts assert things they never measured

**Date:** 2026-08-21
**Subject:** `plan/beads-v1-1-2-upgrade/rehearsal-package/wrappers/`
**Finding:** four producer scripts publish **eight** assertion fields as a
...
in one place; four other producers violate it.
...
This is the most serious of the four: it is the receipt that would be cited to
...
green, and that green carries no information about the four assertions above.
...
cross the version boundary. This is sharper: four of its checks would report
...
is a working template for all four.
...
less while four of its existing checks cannot fail.

## Scope

Read every producer under `rehearsal-package/wrappers/` that writes a receipt,
and every schema under `rehearsal-package/schemas/`. The classification above is
per-script and each is quoted. Not run — this is a source reading of a package
whose execution needs a privileged host, so "cannot fail" is a claim about the
code, argued from the arguments each script receives.
