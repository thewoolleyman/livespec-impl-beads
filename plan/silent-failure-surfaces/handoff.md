...
| `bd-ib-2os2` | this repo | **closed** 2026-08-22, PR #1736 | `beads_access_guard` blocks a `git commit -m` whose message mentions the ledger tool as a bare word, because the matcher tests raw text rather than command position. |
...
while `bd-ib-nf39` is a third local child and is still open. The
epic-disposition decision is therefore **not yet live**. The plan is
deliberately **not archived** — archiving would assert a completeness the
stated member set does not have.
