...
test_passthrough_streams_tty_and_signal() {
    ...
    (
        ...
        grep -qF "does-not-exist" guard-missing.err || fail "unknown-command stderr was not preserved"
        ...
        timeout -s TERM 15s env \
            ...
            "$WRAPPER" list --status all --limit 0 --json >signal.out 2>signal.err
        signal_rc=$?
        set -e
        [ "$signal_rc" -ne 124 ] && [ "$signal_rc" -ne 137 ] \
            || fail "signal/foreground probe timed out or was killed"
    )
...
test_create_normalization() {
    ...
    (
        ...
        if python3 - after-ephemeral.json <<'PY'
import json
import sys

raise SystemExit(any(item["id"] == "q-ephemeral" for item in json.load(open(sys.argv[1], encoding="utf-8"))))
PY
        then
            :
        else
...
PY
        ...
        run_guard create --id q-batch --file missing-plan.md >batch.out 2>batch.err || true
        run_bd list --status all --limit 0 --json >after-batch.json
        if python3 - after-batch.json <<'PY'
import json
import sys

raise SystemExit(any(item["id"] == "q-batch" for item in json.load(open(sys.argv[1], encoding="utf-8"))))
PY
        then
            :
        else
            fail "batch create created or normalized a single issue"
        fi
        ...
        if python3 - after-local-selector.json <<'PY'
import json
import sys

raise SystemExit(any(item["id"] == "q-selector" for item in json.load(open(sys.argv[1], encoding="utf-8"))))
PY
        then
            :
        else
