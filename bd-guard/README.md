...
binary. It creates only throwaway repositories under `mktemp`, runs with a
temporary `HOME` and `XDG_CONFIG_HOME`, and never copies anything to
`/usr/local/bin`, contacts a production tenant, builds an image, or touches the
