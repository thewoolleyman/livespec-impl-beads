...
description: Open an interactive stand-by session over one work item or plan — assemble its context through the context primitive, answer questions, draft research, record maintainer rulings as plan scope events, and drive a lifecycle action only on explicit instruction. Invoked as livespec-orchestrator-beads-fabro:discuss-work-item with a plan_slug or work_item_id.
---

# discuss-work-item — Codex binding

Thin Codex binding for the `discuss-work-item` operation of the
**livespec-orchestrator-beads-fabro** plugin. The complete
harness-neutral driving prose — the subject resolution, the
...
the plugin's own artifact at `$PLUGIN_ROOT/prose/discuss-work-item.md`.
FIRST resolve `$PLUGIN_ROOT` (next section), THEN read that prose file
in full, then execute it end-to-end, binding its harness-neutral
vocabulary to this runtime per `## Runtime bindings` below. This binding
adds NO operation behavior of its own.
...
```bash
cat "$PLUGIN_ROOT/prose/discuss-work-item.md"
```
...
- **"ask the user" / "confirm with the user" / "obtain consent" /
  "surface" / "narrate" / "stand by and ask"** — conversational
  narration in this session (ask one question at a time).
- **"read `<file>`"** — reading the file directly. **"write `<file>`"** —
  writing the file directly. **Enumerate `plan/` directories** — a
  directory listing. **Python snippets** — run via the shell tool
...
- **"assemble the context envelope" / "invoke the `context`
  primitive"** — the `context` skill in this plugin (invoke it by name),
  or its wrapper run via the shell tool with `--json`:
...
- **"the `drive` operation"** — the `drive` skill in this plugin (the
  one lifecycle-action seam; invoke it by name, and only on an explicit
...
- **"the `capture-work-item` operation"** — the `capture-work-item`
  skill in this plugin (the ripe-work filing seam; invoke it by name).
- **"the `doctor` / `propose-change` operation"** — the cross-boundary
  `doctor` and `propose-change` skills of the **livespec** plugin
  (invoke them by name).
