---
name: context
description: Assemble one work-item's or plan epic's full context envelope from the beads-backed store (read-only thin transport). Invoked as livespec-orchestrator-beads-fabro:context.
---

# context — Codex binding

Thin Codex binding for `context`. The behavior lives in the plugin's
reference wrapper `scripts/bin/context.py`; this binding only resolves
the plugin root and dispatches. Read-only pass-through — it never
mutates the store.
...
Supported flags mirror the wrapper: the required
`<plan_slug | work_item_id>` positional, `--json`,
`--project-root <path>`, `--work-items-path <path>`.

## Output

Surface the wrapper's stdout verbatim — the context envelope as one JSON
object when `--json` is passed. Do NOT re-interpret or mutate any state.
An absent id or `plan_slug` exits 3 naming the missing key; surface that
verbatim rather than retrying with a guessed key.
