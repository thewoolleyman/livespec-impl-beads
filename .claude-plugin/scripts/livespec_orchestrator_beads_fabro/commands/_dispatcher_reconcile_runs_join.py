...
def classify_orphans(
    *,
    ...
    for row in attributed_runs(inventory=inventory):
        reading = _reason_with_grace(row=row, grace=grace)
...
def blocked_grace_candidate_ids(*, inventory: FactoryRunInventory) -> tuple[str, ...]:
    ...
    """
    return tuple(
        row.run.run_id for row in attributed_runs(inventory=inventory) if _grace_governed(row=row)
    )
...
def classify_blocked_holds(
    *,
    ...
    held: list[HeldRun] = []
    for row in attributed_runs(inventory=inventory):
        if not _grace_governed(row=row):
            continue
...
def _grace_governed(*, row: AttributedRun) -> bool:
    return (
...
def _reason_with_grace(*, row: AttributedRun, grace: BlockedRunGrace | None) -> _GraceReading:
    ...
    """
    if grace is None or grace.grace_seconds <= 0 or not _grace_governed(row=row):
