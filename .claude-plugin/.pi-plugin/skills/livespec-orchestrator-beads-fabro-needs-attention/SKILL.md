---
name: livespec-orchestrator-beads-fabro-needs-attention
description: Compose the specification, implementation, human-valve, plan, and hygiene signals of this repository into one attention list. Use when the user asks what needs attention or wants a selectable operator action id. Read-only.
allowed-tools: bash
---

# livespec-orchestrator-beads-fabro-needs-attention — pi binding

This file is the thin pi binding of the `needs-attention` operation of the
...
`scripts/bin/needs_attention.py`; this binding resolves the plugin root and
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:needs-attention` form the Claude and Codex
