...
# livespec-orchestrator-beads-fabro-capture-work-item — pi binding

This file is the thin pi binding of the `capture-work-item` operation of the
...
plugin's shared artifact at `prose/capture-work-item.md`, the same artifact the
Claude and Codex bindings read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/capture-work-item.md` **completely** with the `read` tool.
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:capture-work-item` form the Claude and Codex
surfaces use.
