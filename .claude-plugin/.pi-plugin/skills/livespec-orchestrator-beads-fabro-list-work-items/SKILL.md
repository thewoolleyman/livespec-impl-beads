---
name: livespec-orchestrator-beads-fabro-list-work-items
description: List work-items from the beads-backed work-items store, optionally filtered by lane, origin, gap id, or spec-commitment hint. Use when the user asks what work exists or wants the store enumerated. Read-only.
allowed-tools: bash
---

# livespec-orchestrator-beads-fabro-list-work-items — pi binding

This file is the thin pi binding of the `list-work-items` operation of the
...
`scripts/bin/list_work_items.py`; this binding resolves the plugin root and
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:list-work-items` form the Claude and Codex
