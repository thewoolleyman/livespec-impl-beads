...
# livespec-orchestrator-beads-fabro-capture-spec-drift — pi binding
...
2. Read `$PLUGIN_ROOT/prose/capture-spec-drift.md` **completely** with the `read` tool.
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:capture-spec-drift` form the Claude and Codex
surfaces use.
