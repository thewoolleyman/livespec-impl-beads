...
description: "File a gap-tied work-item per gap over the gap-id set emitted by the sibling detect-impl-gaps skill, with per-gap user consent. That sibling is a spec-clause enumerator, not a spec→impl comparator — it never reads implementation state, so a captured gap-id means the clause is not yet tracked by a work-item, never that it is missing from the implementation. Use when the user wants enumerated spec clauses turned into tracked work. Mutating: it writes one record per consented gap."
allowed-tools: bash read
---

# livespec-orchestrator-beads-fabro-capture-impl-gaps — pi binding

This file is the thin pi binding of the `capture-impl-gaps` operation of the
...
plugin's shared artifact at `prose/capture-impl-gaps.md`, the same artifact the
Claude and Codex bindings read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/capture-impl-gaps.md` **completely** with the `read` tool.
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:capture-impl-gaps` form the Claude and Codex
surfaces use.
