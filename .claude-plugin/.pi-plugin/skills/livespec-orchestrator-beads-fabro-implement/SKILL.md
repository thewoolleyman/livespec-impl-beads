...
# livespec-orchestrator-beads-fabro-implement — pi binding

This file is the thin pi binding of the `implement` operation of the
...
plugin's shared artifact at `prose/implement.md`, the same artifact the
Claude and Codex bindings read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/implement.md` **completely** with the `read` tool.
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:implement` form the Claude and Codex
surfaces use.
