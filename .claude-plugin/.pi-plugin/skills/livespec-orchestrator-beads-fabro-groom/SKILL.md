---
name: livespec-orchestrator-beads-fabro-groom
description: Regroom an oversized or non-converging work-item into ready, dependency-layered slices. Use when an item keeps failing to converge or is too large to dispatch. Drafting is read-only; the maintainer owns the cut and nothing is filed until they approve it.
allowed-tools: bash read
---

# livespec-orchestrator-beads-fabro-groom — pi binding

This file is the thin pi binding of the `groom` operation of the
...
plugin's shared artifact at `prose/groom.md`, the same artifact the
Claude and Codex bindings read.

Order of work, every time:

1. Resolve `$PLUGIN_ROOT` (next section).
2. Read `$PLUGIN_ROOT/prose/groom.md` **completely** with the `read` tool.
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:groom` form the Claude and Codex
surfaces use.
