---
name: livespec-orchestrator-beads-fabro-list-plans
description: List the unarchived plans held in this repository's plan store. Use when the user asks which plans are open or wants to resume one. Read-only.
allowed-tools: bash
---

# livespec-orchestrator-beads-fabro-list-plans — pi binding

This file is the thin pi binding of the `list-plans` operation of the
...
`scripts/bin/list_plans.py`; this binding resolves the plugin root and
...
prefix rather than by the `/livespec-orchestrator-beads-fabro:list-plans` form the Claude and Codex
