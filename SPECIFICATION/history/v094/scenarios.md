...
Feature: Every governed-repo integration point is a field of one typed schema, read by one
  ...
  Scenario: A token in a position the engine does not render fails the seam-equivalence check
    Given the implement-work-item workflow places an inputs token in an attribute the engine does not template
    When the CI seam-equivalence check runs
    Then the check fails naming the token and its non-rendered position
```
