...
## Scenario 96 — Each governed-repo integration point resolves from a committed declaration or the fleet default

```gherkin
Feature: The janitor check-suite and janitor-core provisioning resolve from what the
  repository DECLARES, running a declared command verbatim and falling back to the fleet
  default only when the key is truly absent, so a member and an adopter consume the
