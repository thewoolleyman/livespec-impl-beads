...
identical, and every token MUST sit in a position the engine renders; a CI
check MUST enforce this equivalence, so a templating question is answered
statically rather than by a production dispatch.
...
MUST NOT be tailored to any single provider's observed behavior. The one
signal the Dispatcher trusts is a dispatch outcome: a SUCCESSFUL dispatch
...
governs the CREATION of the record; this rule governs its RETIREMENT, and the
two are the same principle applied at both ends.
