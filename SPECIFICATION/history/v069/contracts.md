...
in still inherits the pin. Resolution MUST degrade per key rather than wholesale:
an absent block, an absent tier entry, a non-table entry, or an absent key within
a tier MUST each fall back to the built-in default for exactly what is missing,
leaving any sibling override in force. A partial override is legal.
...
So the implementer adapter a default dispatch renders is, in full:
...
reader can reconstruct the literal string and check it against `run_turn.command`
without reading the implementation. The implementer holds the stronger MODEL and
cuts only its reasoning effort, because a weaker implementer buys token savings
at the cost of extra `review_fix` rounds, which are themselves Codex turns; the
`pr` node runs a scripted recipe and takes the cheap model outright.
