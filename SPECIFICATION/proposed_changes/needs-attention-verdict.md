...
- `bd-ib-tfpdya` (pending-approval) records that the criteria evaluator judges raw `splitlines()` fragments, so wrapped continuation lines become standalone false-failing "criteria" — and, for the WALLS below, count as gradeable lines that mask a genuinely empty criteria set. Reviewer fable's finding 4(a) is binding: building new refusal points on the defective parse institutionalizes it. The primitive below is therefore ratified at the ASSERTION level (the line-based parse is the current, known-defective approximation; its repair IS `bd-ib-tfpdya`), and the implementation children of this proposal's walls MUST land together with or after that repair.
...
Replace, in the `accept` bullet, "yielding a PASS or FAIL verdict" with "yielding a PASS, FAIL, or NEEDS_ATTENTION verdict", and append after that bullet's policy list:
...
>
> 1. The item's native `acceptance_criteria` field, when it yields gradeable content.
> 2. Otherwise the metadata-merged read (the same merged store path the acceptance pass uses — a criteria field held in item metadata by an older writer MUST NOT be treated as absent).
> 3. Otherwise the item description's "Exit criteria" section (a heading whose title case-insensitively equals "Exit criteria"; the section body is the criteria text).
>
> Gradeability is defined at the ASSERTION level: an effective-criteria set is empty when it contains zero gradeable assertions. A physical-line parse that counts wrapped continuation fragments as assertions is a known-defective approximation of this definition (`bd-ib-tfpdya`); the walls below MUST NOT be implemented against a parse that counts non-assertable fragments as gradeable — their implementation lands together with or after that repair.

#### 4. `contracts.md` — the two walls

Append to §"Admission valve (`ready -> active`)" a new bullet in the mechanical-conditions list, and a matching clause at the `approve` transition in §"Work-item state semantics":
...
Update the dispatcher exit-code documentation (the ratified exit-code enumeration) to add: `5` — ungradeable-acceptance-criteria refusal.
...
Scenario: Criteria held in metadata or a description Exit criteria section pass the wall
  Given an item whose criteria live only in the metadata-merged read or in a description section titled Exit criteria
  When the walls evaluate it
  Then it is dispatched normally rather than refused
