...
def test_owning_repo_root_prefers_claude_project_dir(
    ...
    assert hook.owning_repo_root() == project.resolve()


def test_gate_state_uses_unresolved_project_dir_when_resolve_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project = tmp_path / "consuming-project"
    project.mkdir()
    ...
    monkeypatch.setattr(hook.Path, "resolve", boom)

    assert hook.gate_state(env={"CLAUDE_PROJECT_DIR": str(project)}) == "on"


def test_owning_repo_root_falls_back_to_dot_when_file_and_cwd_resolution_fail(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def boom(*_args: object, **_kwargs: object) -> Path:
    ...
    monkeypatch.setattr(hook.Path, "resolve", boom)
    monkeypatch.setattr(hook.Path, "cwd", boom)

    assert hook.owning_repo_root() == hook.Path(".")
