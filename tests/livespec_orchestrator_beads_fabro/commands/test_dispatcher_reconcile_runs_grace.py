...
class _Runner:
    """A fake `fabro` CLI that answers `inspect` and records every argv."""
    ...
    def run(
        ...
        *,
        ...
        return CommandResult(exit_code=0, stdout=self.inspect_by_run.get(argv[2], "[]"), stderr="")
...
def test_measuring_a_park_costs_one_inspect_per_named_run(tmp_path: Path) -> None:
    module = importlib.import_module(_MODULE_NAME)
    runner = _Runner(
        inspect_by_run={
            "01PARKED": json.dumps([{"blocked_at": 0}]),
            "01OPAQUE": json.dumps([{"status": {"kind": "blocked"}}]),
        }
    )

    grace = module.measured_grace(
        port=_port(runner=runner, tmp_path=tmp_path),
        run_ids=("01PARKED", "01OPAQUE"),
        grace_seconds=1800,
        now_epoch=1801.0,
    )

    assert grace.grace_seconds == 1800
    assert grace.parked_seconds_by_run == {"01PARKED": 1801.0, "01OPAQUE": None}
    assert [call[1] for call in runner.calls] == ["inspect", "inspect"]


def test_a_zero_grace_measures_nothing_at_all(tmp_path: Path) -> None:
    module = importlib.import_module(_MODULE_NAME)
    runner = _Runner()
...
def test_the_wall_clock_is_read_when_no_instant_is_supplied(tmp_path: Path) -> None:
    module = importlib.import_module(_MODULE_NAME)
    runner = _Runner(inspect_by_run={"01PARKED": json.dumps([{"blocked_at": 0}])})
    ...
    parked = grace.parked_seconds_by_run["01PARKED"]
...
def _port(*, runner: _Runner, tmp_path: Path) -> FabroPort:
    return FabroPort(
        fabro_bin="fabro",
        target=FabroTarget(server_url="https://hp.example:32276", dev_token=None),
        runner=runner,
        cwd=tmp_path,
    )
