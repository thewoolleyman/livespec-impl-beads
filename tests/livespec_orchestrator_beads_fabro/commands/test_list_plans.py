"""Scenario 42 coverage for the list-plans thin-transport command."""

import json

import pytest
from livespec_orchestrator_beads_fabro.commands.list_plans import (
    list_plans,
    main,
)


def test_scenario42_json_lists_unarchived_plans_in_lexicographic_order(
    tmp_path,
...
def test_list_plans_ignores_files_and_archive_directory(tmp_path) -> None:
    plan = tmp_path / "plan"
    _ = (plan / "topic").mkdir(parents=True)
    (plan / "notes.md").write_text("not a plan\n", encoding="utf-8")
    _ = (plan / "archive").mkdir()

    assert list_plans(project_root=tmp_path) == ["topic"]
